...
class Main:
    def __init__(self, master):
        ...
        getKey = Tkinter.Button(buttons,image=self.igmGK)
        getKey.pack(side=Tkinter.LEFT)
        getKey.bind("<ButtonRelease-1>",self.openScreenGetKey)
        returnKey = Tkinter.Button(buttons,image=self.igmRK)
...
class ScreenGetKey:
    ...
    def pressButton(self, event):
        ...
        askquestion = tkMessageBox.askquestion("Confirm",self._geom()
                                             "Get key from room " + str(event.widget.data[1]) + "?" )
